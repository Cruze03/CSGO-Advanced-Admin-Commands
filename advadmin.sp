...
//	RegAdminCmd("sm_spec",			CMD_Spec,			ADMFLAG_KICK,		"Set the targets team to spectator");
	RegAdminCmd("sm_spectator",		CMD_Spec,			ADMFLAG_KICK,		"Set the targets team to spectator");
	RegAdminCmd("sm_spectate",		CMD_Spec,			ADMFLAG_KICK,		"Set the targets team to spectator");
	...
	RegAdminCmd("sm_strip",			CMD_Disarm,			ADMFLAG_BAN,		"Disarming the targets");
   	RegAdminCmd("sm_drop",			CMD_Drop, 			ADMFLAG_GENERIC, 	"Force to drop the specified weapon");
	//-----//
	...
	RegAdminCmd("sm_money",			CMD_Cash,			ADMFLAG_BAN,		"Set the cash for the targets");
	//-----//
	...
	RegAdminCmd("sm_getmodels",		CMD_GetModel,		ADMFLAG_BAN,		"Get target's model name");
...
public Action CMD_Drop(int client, int args)
{
	if(args != 2)
	{
		ReplyToCommand(client, "%t", "CMD_Drop_Usage");
		return Plugin_Handled;
	}
	...
	int weaponSlot = StringToInt(buffer2);

	for(int i = 0; i < target_count; i++)
	{
		int weapon = GetPlayerWeaponSlot(target_list[i], weaponSlot);
		if(weapon == -1)
		{
			continue;
		}
		CS_DropWeapon(target_list[i], weapon, true, true);
	}

	if(tn_is_ml)
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_Drop", target_name);
		LogActionEx(client, "%t", "CMD_Drop", target_name);
	}
	else
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_Drop", "_s", target_name);
		LogActionEx(client, "%t", "CMD_Drop", "_s", target_name);
	}
	return Plugin_Handled;
}
...
public Action CMD_God(int client, int args)
{
	...
	if(tn_is_ml)
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_God", target_name, mode);
		LogActionEx(client, "%t", "CMD_God", target_name, mode);
	}
	else
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_God", "_s", target_name, mode);
		LogActionEx(client, "%t", "CMD_God", "_s", target_name, mode);
	}
	return Plugin_Handled;
}
...
public Action CMD_Curse(int client, int args) 
{
	...
	if(tn_is_ml) 
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_Curse", target_name);
	} 
	else 
	{
		ShowActivity2(client, CMD_PREFIX, "%t", "CMD_Curse", "_s", target_name);
	}
